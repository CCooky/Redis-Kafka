...
- 生产者：
  - 采用同步发送，不要使用异步发送，因为异步当没有收到ack时不会进行重试；
  - 设置同步的ack参数为1/all，视情况而定；
  - 设置重试次数retries为一个较大的值，一般是3
- 消费者：
  - 采用手动提交，因为自动提交可能出现消息未消费完就提交了位移的情况
  - 采用同步+异步结合的策略，同样是异步出问题后不会重试提交位移，我们只要保证最后一次提交位移一定成功即可。

**2、Broker端方面**

- 设置``unclean.leader.election.enable = false`。关闭Unclean领导者选举，即ISR集合为空时，报错出现副本故障。
- 设置副本数量``replication.factor >= 3`。最好将消息多保存几份。
- 设置``min.insync.replicas > 1`。控制的是消息至少要被写入到多少个副本才算是“已提交”。和前面的第二点中acks=all配置是相辅相成的作用



## 6.2 各个词的对应关系

- Leader副本选举：ISR、Unclean领导者选举、同步标准——zookeeper和controller负责
- 消费组的Rebalance：Coordinator负责：和Consumer_offset位移管理有关。
- Rebalance：心跳机制、joinGroup请求、SyncGroup请求。
